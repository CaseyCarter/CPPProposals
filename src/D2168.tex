...
that defines a partial or
explicit specialization of \tcode{generator}
is undefined.
...
public:
    ...
    template<class T2, class Ref2, class Alloc2>
        requires same_as<conditional_t<is_reference_v<Ref2>, Ref2, const Ref2&>, yielded>
    auto yield_value(ranges::elements_of<generator<T2, Reference, Alloc2>> g) noexcept;
...
auto yield_value(elements_of<generator<Reference, TVal, TAlloc>> g) noexcept;
\end{itemdecl}

\begin{itemdescr}
\expects
\tcode{TAlloc} meets the \tcode{Cpp17Allocator} requirements.
...
requires convertible_to<ranges::range_reference_t<R>, Reference>
...
\begin{codeblock}
{
    ...
    -> generator<Reference, T, Allocator> {
...
template<class Reference, class T, class Allocator>
    requires common_reference_with<Reference&&, T&>
class generator<Reference, T, Allocator>::@\exposid{iterator}@ {
private:
...
\begin{itemdescr}
\expects \tcode{coroutine_.done()} is \tcode{false}.
