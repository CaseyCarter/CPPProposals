...
In R5, \tcode{generator}'s first parameter \tcode{T} names
...
\begin{itemize}
\item \tcode{coroutine_traits<generator<Reference, T, Allocator>>::promise_type} is valid and denotes a type,
...
public:
    ...
    @\unspec@ yield_value(const Reference& value) noexcept(is_nothrow_move_constructible_v<Reference>);

    template<convertible_to<Reference> T>
    @\unspec@ yield_value(T&& x) noexcept(is_nothrow_constructible_v<Reference, T>);

    template<class TVal, class TAlloc>
    @\unspec@ yield_value(elements_of<generator<Reference, TVal, TAlloc>> g) noexcept; // \seebelownc
...
\begin{itemdecl}
@\unspec@ yield_value(const Reference& x)
    noexcept(is_nothrow_move_constructible_v<Reference>);
\end{itemdecl}
\begin{itemdecl}
template<convertible_to<Reference> T>
@\unspec@ yield_value(T&& x)
    noexcept(is_nothrow_constructible_v<Reference, T>);
\end{itemdecl}
