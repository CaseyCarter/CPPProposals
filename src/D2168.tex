...
\item Remove concerns about the $O(1)$ destruction requirement for \tcode{view},
...
\item a \tcode{Value} type, whose default is derived from the Reference type, and which has a \tcode{common\_reference} with the reference type
...
Instead we propose that for a \tcode{generator<T>},
...
%% FIXME: That isn't precisely what's happening here.
...
namespace std {
    template<class Ref, class Value = remove_cvref_t<Ref>, class Allocator = void>
        requires common_reference_with<Ref&&, Value&>@\footnote{FIXME: this is insufficient to guarantee that \tcode{iterator} is \tcode{indirectly_readable}.}@
    class generator;
...
and \tcode{started_} with \tcode{exchange(other.started_, false)}.
\end{itemdescr}
...
\begin{thebibliography}{9}
    ...
    \url{https://godbolt.org/z/oeKz8d19a}
