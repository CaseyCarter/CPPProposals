...
an stack of associated coroutine handles.
This stack is used to track which coroutine is currently active
when one coroutine recursively yields another.
...
The stack of associated coroutine handles is transferred from \tcode{other} to \tcode{*this}.
...
\begin{itemdescr}
\effects
Equivalent to \tcode{swap(\exposid{coroutine_}, other.\exposid{coroutine_})}.
...
and whose stack of associated coroutine handles is empty.
\end{itemdescr}
...
\begin{itemdescr}
\expects
\exposid{coroutine_} is in the stack of coroutine handles
associated with some \tcode{generator} and
the coroutine handle at the top of that \tcode{generator}'s stack
refers to a suspended coroutine with promise object \tcode{p}.
...
\expects
\exposid{coroutine_} is in the stack of coroutine handles
associated with some \tcode{generator}.
Let \tcode{h} be the coroutine handle
at the top of that \tcode{generator}'s stack
of associated coroutine handles.

\effects
Equivalent to \tcode{h.resume()}.
