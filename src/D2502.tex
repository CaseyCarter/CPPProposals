...
\item Change directions to the Editor from blue ``Drafting Note'' to
...
\item Changes to wording for \tcode{elements_of}:
  ...
  \item Don't reuse the same input range in the example, which could
  ...
  \item Annotate member \tcode{range}
...
\item Rename \tcode{generator}'s first template parameter
  ...
  are usually \tcode{R}.
\item \tcode{generator} publicly derives from \tcode{ranges::view_interface}
  instead of specializing \tcode{ranges::\brk{}enable_view}.
...
\item Allocators must use real pointers; both
  \tcode{generator}'s template parameter \tcode{Allocator} and
  any allocator used in an \tcode{operator new}.
...
\item \tcode{generator} moves don't invalidate iterators.
...
\item Make \tcode{operator==} a hidden friend instead of a member
  ...
  Also use "Effects: equivalent to" for \tcode{operator==}
  to avoid explicit preconditions.
\item Add ``top of stack coroutine is suspended'' preconditions to
  \tcode{iterator::operator*} (explicitly) and
  \tcode{iterator::operator++} (implicit via effects-equivalent-to).
\item Since all ``parameter preview'' arguments
  to \tcode{operator new} are lvalues,
  \tcode{operator new} overloads now take
  the allocator, implicit object, and "ignored" parameters
  by \tcode{const\&}.
\item Make the ``stack of associated coroutines'' an exposition-only
  \tcode{unique_ptr<stack<coroutine\brk{}_handle<>>>} member of \tcode{generator}.
\item Don't use ``Preconditions'' for a template parameter type requirement.
\item (Not requested by LWG) Change implementation of generic
  \tcode{yield_value} per Lewis's suggestion to encourage HALO. (Yes, this
  overload is overspecified.)
\item Add missing \returns element to \tcode{iterator::operator=}.
\item Fix ``\returns \tcode{return *this;}'' for \tcode{iterator::operator++()}.
\item Define the nested classes within \tcode{namespace std}.
\item Update implementation.
\end{itemize}
...
Class template \tcode{generator} presents a view of the elements yielded
...
\mandates
\begin{itemize}
\item If \tcode{Allocator} is not \tcode{void},
...
it shall meet the \tcode{Cpp17Allocator} requirements.
...
\begin{itemdescr}
\effects
Initializes \tcode{\exposid{coroutine_}} with
\tcode{exchange(other.\exposid{coroutine_}, \{\})}
and \tcode{\exposid{active_}} with
\brk{}\tcode{exchange(other.active_, nullptr)}.
...
\effects
Pushes \tcode{\exposid{coroutine_}} into \tcode{*\exposid{active_}},
...
and whose member \tcode{\exposid{active_}} points to an empty stack.
\end{itemdescr}
...
\tcode{g.range\brk{}.\exposid{coroutine_}}
...
\item \tcode{Alloc} for the overloads
...
\begin{itemdescr}
\effects
Equivalent to \tcode{\exposid{coroutine_} = exchange(other.\exposid{coroutine_}, \{\})}.
...
\tcode{\exposid{coroutine_}} is in \tcode{*x.\exposid{active_}} and
...
\expects
For some \tcode{generator} object \tcode{x},
\tcode{\exposid{coroutine_}} is in \tcode{*x.\exposid{active_}}.
...
\begin{thebibliography}{9}
    ...
    \url{https://godbolt.org/z/5hcaPcfvP}
\end{thebibliography}
