...
\item Reorder \tcode{generator}'s first two template parameters,
  ...
  which is both consistent with the D2168R4 design and allows
...
\tcode{generator<T, Reference = const T\&, Allocator = void>}

\begin{itemize}
\item a type \tcode{T},
  a \cv-unqualified object type that specifies the value type
  of the \tcode{generator}'s range and iterators,
\item a type \tcode{Reference} that specifies the reference type
  of the \tcode{generator}'s range and iterators, and
\item a type \tcode{Allocator} type that is rebound to allocate the coroutine state,
...
\begin{note}
Before PXXXXR0, \tcode{generator}'s first parameter
...
\subsubsection{Keeping the D2168R4 design}
...
It is unfortunate that the D2168R4 design silently alters
...
In PXXXXR0, \tcode{generator}'s first parameter \tcode{T} names
...
stash a the value in an awaitable returned from \tcode{yield_value}.

This approach keeps the desirable features of the D2168R4 design,
...
\tcode{std::generator<int, const int\&, std::stateless_allocator<\brk{}int>>}
...
Returns a generator of type \tcode{std::generator<int, const int\&, some_stateful_allocator<\brk{}int>>}
...
\mandates \tcode{T} is a cv-unqualified object type.

\mandates \tcode{Reference} is either a reference type, or
a cv-unqualified object type that models \libconcept{copy_constructible}.

\tcode{Allocator} shall either meet the \tcode{Cpp17Allocator} requirements,
or be \tcode{void}.

\mandates
Let \tcode{RRef} denote \tcode{remove_reference_t<Reference>\&\&}
if \tcode{Reference} is a reference type,
or \tcode{Reference} otherwise.
Each of:
\begin{itemize}
\item \tcode{common_reference_with<Reference\&\&, T\&>},
\item \tcode{common_reference_with<Reference\&\&, RRef\&\&>}, and
\item \tcode{common_reference_with<RRef\&\&, const T\&>}
\end{itemize}
is modeled.
\begin{note}
...
a partial or explicit specialization of \tcode{generator}
is undefined.
...
class generator<T, Reference, Allocator>::promise_type {
  ...
  using @\exposid{yielded}@ = // \expos
    conditional_t<is_reference_v<Reference>, Reference, const Reference&>;

  add_pointer_t<@\exposid{yielded}@> @\exposid{value_}@ = nullptr; // \expos
...
public:
  ...
  suspend_always yield_value(@\exposid{yielded}@ value) noexcept;

  template<class T2, class Ref2, class Alloc2>
    requires same_as<conditional_t<is_reference_v<Ref2>, Ref2, const Ref2&>, @\exposid{yielded}@>
      auto yield_value(ranges::elements_of<generator<T2, Ref2, Alloc2>> g) noexcept;

  template<ranges::input_range R, class Alloc2>
    requires convertible_to<ranges::range_reference_t<R>, @\exposid{yielded}@>
...
\begin{itemdecl}
suspend_always yield_value(@\exposid{yielded}@ x) noexcept;
\end{itemdecl}
...
\begin{codeblock}
    auto nested = [](allocator_arg_t, Alloc2, auto* range_ptr)
      -> generator<ranges::range_value_t<R>, @\exposid{yielded}@, Alloc2> {
        for (auto&& e : *range_ptr)
          co_yield static_cast<@\exposid{yielded}@>(std::forward<decltype(e)>(e));
      };
    auto&& rng = r.range();
...
\begin{codeblock}
template<class T, class Reference, class Allocator>
class generator<T, Reference, Allocator>::@\exposid{iterator}@ {
public:
    using value_type = T;
...
includes \tcode{coroutine_}.

\effects
Equivalent to:
\begin{codeblock}
    return static_cast<Reference>(*p.@\exposid{value_}@);
\end{codeblock}
\end{itemdescr}
...
\begin{thebibliography}{9}
    ...
    \url{https://godbolt.org/z/Y4qa386Pa}
