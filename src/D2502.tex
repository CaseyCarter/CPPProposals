...
\item \tcode{generator} publicly derives from \tcode{ranges::view_interface}
  instead of specializing \tcode{ranges::enable_view}.
...
\item Make ``stack of associated coroutine handles'' an explicit
  \tcode{unique_ptr<stack<coroutine_handle<>>>} member of \tcode{generator}.
...
\item If \tcode{Allocator} is not \tcode{void},
  \tcode{allocator_traits<Allocator>::pointer} has pointer type.
...
and \tcode{\exposid{active_}} with \tcode{exchange(other.active_, nullptr)}.
{*this}.
...
is at the top of \tcode{*\exposid{active_}}
of some \tcode{generator} object \tcode{x}.
The coroutine referred to by \tcode{g.range.\exposid{coroutine_}}
...
Equivalent to:
\begin{codeblock}
    auto nested = [](allocator_arg_t, Alloc, ranges::range_iterator_t<Rng> i,
                     ranges::range_sentinel_t<Rng> s)
...
\mandates
\tcode{allocator_traits<B>::pointer} has pointer type.
