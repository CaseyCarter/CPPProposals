...
using Reference =
  conditional_t<is_void_v<V>,
    conditional_t<is_reference_v<R>, R, const R&>,
    R>;
...
with an xvalue reference type when the lvalue is copyable. The result is that
the yielded lvalue is copied into temporary storage, and an iterator for the
generator will return an xvalue denoting that copy.
...
In practice, this should make \tcode{generator<R, void, my_allocator>}
appear quite rarely in real code.
...
namespace std {
  ...
  inline constexpr bool ranges::enable_view<generator<R, V, Allocator>> = true;
}
\end{codeblock}
...
shall meet the \tcode{Cpp17Allocator} requirements.
...
Let \tcode{r} be an lvalue that denotes
the same object to which \tcode{g.range} refers.

\returns
An object of an unspecified awaitable type~([expr.await])
which takes ownership of the generator \tcode{r},
whose member \tcode{await_suspend}
pushes \tcode{r.\exposid{coroutine_}} atop the stack of
coroutines associated with \tcode{x}
before resuming execution of \tcode{r.\exposid{coroutine_}}, and
whose member \tcode{await_resume} rethrows any exception captured by
a call to \tcode{r.\exposid{coroutine_}.promise()}'s member
\tcode{unhandled_exception}.

\begin{note}
Variables with automatic storage duration in the scope of
the coroutine represented by \tcode{r.\exposid{coroutine_}} are destroyed
...
\begin{codeblock}
template<class R, class V, class Allocator>
class generator<R, V, Allocator>::@\exposid{iterator}@ {
public:
...
\begin{thebibliography}{9}
    ...
    \url{https://godbolt.org/z/P371zhbqd}
